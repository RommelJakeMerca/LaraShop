...
<div class="modal fade" id="approveModalPop" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form action="/approve_order/{{ $latest_order->id }}" method="POST">
          {{ csrf_field() }}
          {{ method_field('PUT') }}
...
<div class="modal fade" id="declineModalPop" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form action="/decline_order/{{ $latest_order->id }}" method="POST">
          {{ csrf_field() }}
          {{ method_field('PUT') }}
            <div class="modal-body">
                <div class="form-group">
            ...
            <div class="card-body">
                <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $latest_order->id }}</div>
                    </div>
                    <div class="col mr-2">
                        <div class="text-xs font-weight-bold text-info text-uppercase mb-1">Client ID</div>
                        <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $latest_order->client_id }}</div>
                    </div>
                    <div class="col mr-2">
                        <div class="text-xs font-weight-bold text-info text-uppercase mb-1">Mode of Payment</div>
                        <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $latest_order->mode_of_payment }}</div>
                    </div>
                    <div class="col mr-2">
                        <div class="text-xs font-weight-bold text-info text-uppercase mb-1">Total</div>
                        <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $latest_order->total_payment }}</div>
                    </div>
                    <div class="col mr-2">
                        <div class="text-xs font-weight-bold text-info text-uppercase mb-1">Status</div>
                        @if ($latest_order->status == "For Approval")
                            <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $latest_order->status }}</div>
                        @elseif ($latest_order->status == "Approved")
                            <div class="h5 mb-0 font-weight-bold text-success">{{ $latest_order->status }}</div>
                        @elseif ($latest_order->status == "Declined")
                            <div class="h5 mb-0 font-weight-bold text-danger">{{ $latest_order->status }}</div>
                        @endif
                    </div>
                    <div class="col mr-2">
                        <div class="text-xs font-weight-bold text-info text-uppercase mb-1">Email Status</div>
                        @if ($latest_order->email_status == "Not Sent")
                            <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $latest_order->email_status }}</div>
                        @else
                            <div class="h5 mb-0 font-weight-bold text-success">{{ $latest_order->email_status }}</div>
                        @endif
                    </div>
                    <div class="col-auto">
                    ...
                    <div class="col-auto">
                        @if ($latest_order->email_status == "Not Sent")
                            <a href="/show_qr_of_order/{{ $latest_order->id }}" type="button" class="btn btn-success" style="display: none; float: right; margin-left: 10px;">View QR Code</a>
                        @else
                            <a href="/show_qr_of_order/{{ $latest_order->id }}" type="button" class="btn btn-success" style="float: right; margin-left: 10px;">View QR Code</a>
                        @endif
                    </div>
                </div>
            </div>
