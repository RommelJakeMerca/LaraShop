...
@section('titlePage', 'LegaShop | Canned Goods');
@section('content')
...
<div class="container mt-4">
    <form action="{{route('filterCannedGoods')}}" method="POST">
        @csrf
...
<div class="container product">
    <div class="row product-container">
        @foreach ($productsCannedGoods as $productsCannedGood)
            <div class="col-md-3 product-col" data-id="{{$productsCannedGood->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsCannedGood->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsCannedGood->product_image)}}" class="product-img">
                        <div class="items-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsCannedGood->product_price}}.00</b></p>
                            <p class="card-name">{{$productsCannedGood->product_name}}</p></a>
                            <p class="card-category">{{$productsCannedGood->category_name}}</p>
        ...
        @endforeach
        {{$productsCannedGoods->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
