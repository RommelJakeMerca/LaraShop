...
@section('titlePage', 'LegaShop | Sardines');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsSardines as $productsSardine)
            <div class="col-md-3 product-col" data-id="{{$productsSardine->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsSardine->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsSardine->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsSardine->product_price}}.00</b></p>
                            <p class="card-name">{{$productsSardine->product_name}}</p></a>
                            <p class="card-category">{{$productsSardine->category_name}}</p>
        ...
        @endforeach
        {{$productsSardines->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
