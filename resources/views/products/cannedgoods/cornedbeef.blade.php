...
@section('titlePage', 'LegaShop | Corned Beef');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsCornedBeefs as $productsCornedBeef)
            <div class="col-md-3 product-col" data-id="{{$productsCornedBeef->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsCornedBeef->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsCornedBeef->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsCornedBeef->product_price}}.00</b></p>
                            <p class="card-name">{{$productsCornedBeef->product_name}}</p></a>
                            <p class="card-category">{{$productsCornedBeef->category_name}}</p>
        ...
        @endforeach
        {{$productsCornedBeefs->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
