...
@section('titlePage', 'LegaShop | Softdrinks');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsSoftdrinks as $productsSoftdrink)
            <div class="col-md-3 product-col" data-id="{{$productsSoftdrink->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsSoftdrink->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsSoftdrink->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsSoftdrink->product_price}}.00</b></p>
                            <p class="card-name">{{$productsSoftdrink->product_name}}</p></a>
                            <p class="card-category">{{$productsSoftdrink->category_name}}</p>
        ...
        @endforeach
        {{$productsSoftdrinks->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
