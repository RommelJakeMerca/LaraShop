...
@section('titlePage', 'LegaShop | Biscuit');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsBiscuits as $productsBiscuit)
            <div class="col-md-3 product-col" data-id="{{$productsBiscuit->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsBiscuit->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsBiscuit->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsBiscuit->product_price}}.00</b></p>
                            <p class="card-name">{{$productsBiscuit->product_name}}</p></a>
                            <p class="card-category">{{$productsBiscuit->category_name}}</p>
        ...
        @endforeach
        {{$productsBiscuits->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
