...
@section('titlePage', 'LegaShop | Chips');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsChips as $productsChip)
            <div class="col-md-3 product-col" data-id="{{$productsChip->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsChip->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsChip->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsChip->product_price}}.00</b></p>
                            <p class="card-name">{{$productsChip->product_name}}</p></a>
                            <p class="card-category">{{$productsChip->category_name}}</p>
        ...
        @endforeach
        {{$productsChips->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
