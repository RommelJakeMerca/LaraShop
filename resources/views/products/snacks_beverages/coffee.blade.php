...
@section('titlePage', 'LegaShop | Coffee');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsCoffees as $productsCoffee)
            <div class="col-md-3 product-col" data-id="{{$productsCoffee->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsCoffee->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsCoffee->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsCoffee->product_price}}.00</b></p>
                            <p class="card-name">{{$productsCoffee->product_name}}</p></a>
                            <p class="card-category">{{$productsCoffee->category_name}}</p>
        ...
        @endforeach
        {{$productsCoffees->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
