...
@section('titlePage', 'LegaShop | Beer');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsBeers as $productsBeer)
            <div class="col-md-3 product-col" data-id="{{$productsBeer->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsBeer->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsBeer->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsBeer->product_price}}.00</b></p>
                            <p class="card-name">{{$productsBeer->product_name}}</p></a>
                            <p class="card-category">{{$productsBeer->category_name}}</p>
        ...
        @endforeach
        {{$productsBeers->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
