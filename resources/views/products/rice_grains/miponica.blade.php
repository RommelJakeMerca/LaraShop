...
@section('titlePage', 'LegaShop | Miponica ');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsMiponicaRices as $productsMiponicaRice)
            <div class="col-md-3 product-col" data-id="{{$productsMiponicaRice->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsMiponicaRice->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsMiponicaRice->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsMiponicaRice->product_price}}.00</b></p>
                            <p class="card-name">{{$productsMiponicaRice->product_name}}</p></a>
                            <p class="card-category">{{$productsMiponicaRice->category_name}}</p>
        ...
        @endforeach
        {{$productsMiponicaRices->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
