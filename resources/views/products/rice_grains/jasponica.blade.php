...
@section('titlePage', 'LegaShop | Jasponica');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsJasponicaRices as $productsJasponicaRice)
            <div class="col-md-3 product-col" data-id="{{$productsJasponicaRice->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsJasponicaRice->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsJasponicaRice->product_image)}}"class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsJasponicaRice->product_price}}.00</b></p>
                            <p class="card-name">{{$productsJasponicaRice->product_name}}</p></a>
                            <p class="card-category">{{$productsJasponicaRice->category_name}}</p>
        ...
        @endforeach
        {{$productsJasponicaRices->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
