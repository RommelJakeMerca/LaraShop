...
@section('titlePage', 'LegaShop | Jasmine ');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsJasmineRices as $productsJasmineRice)
            <div class="col-md-3 product-col" data-id="{{$productsJasmineRice->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsJasmineRice->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsJasmineRice->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsJasmineRice->product_price}}.00</b></p>
                            <p class="card-name">{{$productsJasmineRice->product_name}}</p></a>
                            <p class="card-category">{{$productsJasmineRice->category_name}}</p>
        ...
        @endforeach
        {{$productsJasmineRices->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
