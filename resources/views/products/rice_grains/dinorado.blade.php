...
@section('titlePage', 'LegaShop | Dinorado ');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsDinoradoRices as $productsDinoradoRice)
            <div class="col-md-3 product-col"  data-id="{{$productsDinoradoRice->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsDinoradoRice->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsDinoradoRice->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsDinoradoRice->product_price}}.00</b></p>
                            <p class="card-name">{{$productsDinoradoRice->product_name}}</p></a>
                            <p class="card-category">{{$productsDinoradoRice->category_name}}</p>
        ...
        @endforeach
        {{$productsDinoradoRices->links('/vendor/pagination/products-pagination')}}
    </div>
</div>
