...
@section('titlePage', 'LegaShop | Sinandemong ');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsSinandomengRices as $productsSinandomengRice)
            <div class="col-md-3 product-col" data-id="{{$productsSinandomengRice->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsSinandomengRice->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsSinandomengRice->product_image)}}"class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsSinandomengRice->product_price}}.00</b></p>
                            <p class="card-name">{{$productsSinandomengRice->product_name}}</p></a>
                            <p class="card-category">{{$productsSinandomengRice->category_name}}</p>
        ...
        @endforeach
        {{$productsSinandomengRices->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
