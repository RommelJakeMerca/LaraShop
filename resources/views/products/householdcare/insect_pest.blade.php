...
@section('titlePage', 'LegaShop | Insect & Pest Control');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsinsectPestControls as $productsinsectPestControl)
            <div class="col-md-3 product-col" data-id="{{$productsinsectPestControl->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsinsectPestControl->id)}}">
                <div class="card">  
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsinsectPestControl->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsinsectPestControl->product_price}}.00</b></p>
                            <p class="card-name">{{$productsinsectPestControl->product_name}}</p></a>
                            <p class="card-category">{{$productsinsectPestControl->category_name}}</p>
        ...
        @endforeach
        {{$productsinsectPestControls->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
