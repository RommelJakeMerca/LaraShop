...
@section('titlePage', 'LegaShop | Dishwashing');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsDiswashings as $productsDiswashing)
            <div class="col-md-3 product-col" data-id="{{$productsDiswashing->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsDiswashing->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsDiswashing->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsDiswashing->product_price}}.00</b></p>
                            <p class="card-name">{{$productsDiswashing->product_name}}</p></a>
                            <p class="card-category">{{$productsDiswashing->category_name}}</p>
        ...
        @endforeach
        {{$productsDiswashings->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
