...
@section('titlePage', 'LegaShop | Toilet Cleaners');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productstoiletCleaners as $productstoiletCleaner)
            <div class="col-md-3 product-col" data-id="{{$productstoiletCleaner->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productstoiletCleaner->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productstoiletCleaner->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productstoiletCleaner->product_price}}.00</b></p>
                            <p class="card-name">{{$productstoiletCleaner->product_name}}</p></a>
                            <p class="card-category">{{$productstoiletCleaner->category_name}}</p>
        ...
        @endforeach
        {{$productstoiletCleaners->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
