...
@section('titlePage', 'LegaShop | Tissue');
@section('content')
...
<div class="container product">
    <div class="row product-container">
        @foreach($productsTissues as $productsTissue)
            <div class="col-md-3 product-col" data-id="{{$productsTissue->id}}">
                <a class="link-details" href="{{url('/products/product-details/'.$productsTissue->id)}}">
                <div class="card">
                    <div class="card-body">
                        <img src="{{asset('uploads/product_images/'.$productsTissue->product_image)}}" class="product-img">
                        <div class="product-info-container">
                            <p class="card-title mt-4">&#8369; <b id="price-product">{{$productsTissue->product_price}}.00</b></p>
                            <p class="card-name">{{$productsTissue->product_name}}</p></a>
                            <p class="card-category">{{$productsTissue->category_name}}</p>
        ...
        @endforeach
        {{$productsTissues->links('/vendor/pagination/products-pagination')}}
    </div>  
</div>
